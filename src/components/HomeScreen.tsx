...
import { Search, Bell, X, Percent, Tag, Gift } from 'lucide-react';
...
export default function HomeScreen({ onNavigate, onSelectProvider, userCity, hasActiveOrder }: HomeProps) {
    ...
    <div className="flex flex-col h-full w-full bg-bg-main relative">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-24 no-scrollbar">
        ...
        <div 
          ...
          className="mb-8 p-3.5 rounded-2xl bg-gradient-to-r from-accent-primary/20 via-accent-primary/10 to-transparent border border-accent-primary/30 flex items-center justify-between gap-3 cursor-pointer shadow-sm hover:border-accent-primary transition-all"
        >
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <div className="w-9 h-9 shrink-0 rounded-xl bg-accent-primary/20 flex items-center justify-center text-accent-primary font-bold text-sm">
              %
            </div>
            <h4 className="text-[13px] font-bold text-text-primary leading-snug">Special Discounts & Offers</h4>
          </div>
          <div className="shrink-0 flex flex-col items-center justify-center px-3 py-1 rounded-xl bg-accent-primary/15 border border-accent-primary/30 text-accent-primary text-center">
            <span className="text-[11px] font-extrabold leading-tight">View</span>
      ...
      {/* Bottom Nav */}
      <BottomNav activeScreen="home" onNavigate={onNavigate} hasActiveOrder={hasActiveOrder} />
    </div>
  );
}
