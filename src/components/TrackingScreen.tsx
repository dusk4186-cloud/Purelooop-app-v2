import { ArrowLeft, CheckCircle2, Clock, Phone, Star, Bell, X, ChevronRight, Receipt } from 'lucide-react';
...
export default function TrackingScreen({ onNavigate, hasActiveOrder = false, activeOrderDetails = null, paymentMethod = 'card', activeServices = ['WashFold'] }: TrackingProps) {
  ...
  const safeDate = activeOrderDetails?.date || 'Today';
  const safeTime = activeOrderDetails?.time || '09:00 AM';

  const isScheduled = activeOrderDetails?.date === 'Tomorrow' || isOutsideOperatingHours;
  ...
  const safeServices = Array.isArray(activeServices) ? activeServices : ['WashFold'];

  const serviceStatuses = [];
  if (safeServices.includes('WashFold') || safeServices.includes('WashIron')) {
    serviceStatuses.push({ title: 'Washing', desc: 'Pending', active: false, completed: false });
  }
  if (safeServices.includes('DryClean')) {
    serviceStatuses.push({ title: 'Dry Cleaning', desc: 'Pending', active: false, completed: false });
  }
  if (safeServices.includes('Iron') || safeServices.includes('WashIron')) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-bg-main relative">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex flex-col items-center pt-8 pb-8 text-center">
                ...
                <p className="text-sm text-text-secondary max-w-[250px]">
                  {isScheduled 
                    ? `Your booking is confirmed. Pickup is scheduled for ${safeDate.toLowerCase()} at ${safeTime}.` 
      ...
      {/* Bottom Nav */}
      <BottomNav activeScreen="tracking" onNavigate={onNavigate} hasActiveOrder={hasActiveOrder} />
    </div>
  );
}
