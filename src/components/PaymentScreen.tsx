...
export default function PaymentScreen({ 
  ...
  const safeBookingTotal = isNaN(bookingTotal) ? 0 : bookingTotal;
  const finalTotal = safeBookingTotal + preferencesCost;
  const holdAmount = Math.round(finalTotal * 0.15);
  const rawAuthTotal = finalTotal + holdAmount;
  const authorizationTotal = isNaN(rawAuthTotal) ? 0 : rawAuthTotal;
    ...
    <div className="flex flex-col h-full w-full bg-bg-main">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-bg-main border-t border-border-color p-4 pb-12 z-20">
        <button 
          ...
          className="w-full py-4 rounded-xl font-semibold bg-accent-primary text-white dark:text-slate-950 shadow-[0_4px_16px_var(--color-accent-glow)] active:scale-[0.98] transition-transform"
        >
