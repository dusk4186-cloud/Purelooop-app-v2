...
export default function BookingScreen({ 
  ...
  const updateQty = (key: string, delta: number) => {
    ...
    setCartItemsByService((prev: any) => {
      ...
      const defaultItem = INITIAL_ITEMS[key as keyof typeof INITIAL_ITEMS] || { qty: 0, weight: 0.2, name: key, category: 'Top Wear', dryCleanPrice: 99 };
      const existingItem = currentTabItems[key] || defaultItem;
      ...
      return {
        ...
        [activeTab]: {
          ...currentTabItems,
          [key]: { ...defaultItem, ...existingItem, qty: newQty }
        }
      };
    });
  };
  ...
  const updateCustomQty = (index: number, delta: number) => {
    ...
    setCustomItemsByService((prev: any) => {
      ...
      if (updatedList[index]) {
        updatedList[index].qty = Math.max(0, (updatedList[index].qty ?? 0) + delta);
      }
  ...
  Object.keys(cartItemsByService).forEach(serviceId => {
    ...
    if (serviceId === 'DryClean') {
      const itemCost = Object.values(tabItems).reduce((acc: number, item: any) => acc + ((item?.qty ?? 0) * (item?.dryCleanPrice ?? 99)), 0);
      const customCost = tabCustomItems.reduce((acc: number, item: any) => acc + ((item?.qty ?? 0) * 150), 0);
      totalCost += (itemCost + customCost);
    } else {
      const weight = Object.values(tabItems).reduce((acc: number, item: any) => acc + ((item?.qty ?? 0) * (item?.weight ?? 0)), 0);
      totalWeightAllPiles += weight;
      const rate = provider?.serviceRates?.[serviceId] || 80;
      const customQty = tabCustomItems.reduce((acc: number, item: any) => acc + (item?.qty ?? 0), 0);
      totalCost += (weight * rate) + (customQty * 40);
    }
  });

  const totalPrice = isNaN(totalCost) ? 0 : Math.round(totalCost);
    ...
    <div className="flex flex-col h-full w-full bg-bg-main">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-bg-main border-t border-border-color p-4 pb-8 z-20 flex items-center justify-between gap-4">
        ...
        <button 
          ...
          className="flex-1 py-4 rounded-xl font-semibold bg-accent-primary text-white dark:text-slate-950 active:scale-[0.98] transition-transform shadow-[0_4px_16px_var(--color-accent-glow)] disabled:bg-border-color disabled:text-text-secondary disabled:scale-100 disabled:shadow-none"
        >
          Checkout
        </button>
      </div>
