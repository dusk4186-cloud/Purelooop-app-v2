...
export default function AuthScreens({ currentScreen, onNavigate, onSignupAddress, onSignupCity }: AuthProps) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        alert("This email is already registered. Switching to Log In screen!");
        onNavigate('login');
      } else {
  ...
  const isSignupStep2Valid = isPhoneValid && city && pincode.length === 6 && house && street && isValidPincode(city, pincode);
    ...
    <div className="flex flex-col h-full w-full bg-bg-main relative">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-24 no-scrollbar">
        {currentScreen === 'login' && (
          <div className="flex flex-col gap-4 py-6">
            ...
            <button 
              ...
              className="mt-4 w-full py-4 rounded-xl font-semibold bg-accent-primary text-white dark:text-slate-950 disabled:bg-border-color disabled:text-text-secondary disabled:shadow-none transition-all shadow-[0_4px_16px_var(--color-accent-glow)]"
            >
        ...
        {currentScreen === 'signup' && signupStep === 1 && (
          <div className="flex flex-col gap-4 py-6 animate-in fade-in slide-in-from-left-4">
            ...
            <button 
              ...
              className="mt-4 w-full py-4 rounded-xl font-semibold bg-accent-primary text-white dark:text-slate-950 disabled:bg-border-color disabled:text-text-secondary transition-all shadow-[0_4px_16px_var(--color-accent-glow)] disabled:shadow-none flex items-center justify-center gap-2"
            >
              Enter Address ➔
            </button>
        ...
        {currentScreen === 'signup' && signupStep === 2 && (
          <div className="flex flex-col gap-3 py-4 animate-in fade-in slide-in-from-right-4">
            <h4 className="text-sm font-semibold text-text-primary mb-1">
              {auth.currentUser?.providerData?.some(p => p.providerId === 'google.com') ? 'Complete Your Profile & Location' : 'Set Pickup Location'}
            </h4>
            
            <div className="space-y-3.5 mb-4">
              {(auth.currentUser?.providerData?.some(p => p.providerId === 'google.com') || phone.length !== 10) && (
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">Mobile Phone Number *</label>
                  <input 
                    type="tel" 
                    maxLength={10}
                    value={phone}
                    onChange={(e) => setPhone(e.target.value.replace(/\D/g, ''))}
                    className="w-full p-3 rounded-xl bg-bg-elevated border border-border-color text-sm text-text-primary placeholder:text-text-secondary/50 focus:border-action-primary focus:ring-1 focus:ring-action-primary outline-none"
                    placeholder="e.g. 9876543210"
                  />
                  {phone.length > 0 && phone.length !== 10 && (
                    <p className="text-[10px] text-red-500 mt-1">Must be a valid 10-digit mobile number.</p>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full p-3 rounded-xl bg-bg-elevated border text-sm text-text-primary placeholder:text-text-secondary/50 focus:ring-1 outline-none ${!pinValid ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-border-color focus:border-action-primary focus:ring-action-primary'}`}
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 rounded-xl bg-bg-elevated border border-border-color text-sm text-text-primary placeholder:text-text-secondary/50 focus:border-action-primary focus:ring-1 outline-none"
            ...
            <label className="block text-xs font-medium text-text-secondary mb-2 mt-1">Save As</label>
            ...
            <button 
              ...
              className="w-full py-4 rounded-xl font-semibold bg-accent-primary text-white dark:text-slate-950 disabled:bg-border-color disabled:text-text-secondary transition-all shadow-[0_4px_16px_var(--color-accent-glow)] disabled:shadow-none"
            >
              Create Account
            </button>
          </div>
