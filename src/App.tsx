...
export default function App() {
  ...
  const [activeOrderServices, setActiveOrderServices] = useState<string[]>(['WashFold']);

  const getActiveServicesList = () => {
    return Object.keys(cartItemsByService).filter(serviceId => {
       const hasItems = Object.values(cartItemsByService[serviceId] ?? {}).some((item: any) => (item?.qty ?? 0) > 0);
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'home':
        return <HomeScreen onNavigate={setCurrentScreen} hasActiveOrder={hasActiveOrder} onSelectProvider={(id, serviceHint) => {
      ...
      case 'profile':
        return <ProfileScreen onNavigate={setCurrentScreen} hasActiveOrder={hasActiveOrder} />;
      case 'provider':
      ...
      case 'payment':
        return <PaymentScreen 
                  ...
                  onPaymentSuccess={(method) => { 
                    const currentServices = getActiveServicesList();
                    setActiveOrderServices(currentServices.length > 0 ? currentServices : ['WashFold']);
      ...
      case 'tracking':
        return <TrackingScreen 
                  ...
                  activeServices={activeOrderServices} 
               />;
      case 'edit-profile':
